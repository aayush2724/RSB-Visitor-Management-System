...
async function generateQRCode(visitorId) {
  ...
  try {
    ...
    const qrBuffer = await QRCode.toBuffer(approvalUrl, { type: 'png', width: 300 });

    // Upload to Cloudinary
    const result = await uploadToCloudinary(qrBuffer, 'rsb-visitors/qrcodes', `qr-${visitorId}`);
...
app.get('/test-email', async (req, res) => {
  try {
    ...
    res.send(`<h1>Email Test Successful</h1><p>Message ID: ${info.messageId}</p>`);
  } catch (error) {
    res.status(500).send(`<h1>Email Test Failed</h1><pre>${error.message}</pre>`);
  }
});
    ...
    const stats = await Visitor.aggregate([
      {
        $group: {
          ...
          released: {
            $sum: {
              $cond: [
                { $and: [{ $ne: ['$out_time', null] }, { $eq: ['$security_confirmed', true] }] },
                1, 0
              ]
            }
          },
          security_pending: {
            $sum: {
              $cond: [
                { $and: [{ $ne: ['$out_time', null] }, { $eq: ['$security_confirmed', false] }] },
                1, 0
              ]
            }
          },
...
app.get('/api/visitors/export', async (req, res) => {
  try {
    ...
    visitors.forEach(visitor => {
      worksheet.addRow({
        ...visitor,
        status: visitor.scheduled ? 'Scheduled'
          : visitor.security_confirmed ? 'Completed'
          : visitor.out_time ? 'Pending Checkout'
          : 'Active'
      });
    });
...
app.post('/api/schedule', async (req, res) => {
  ...
  try {
    ...
    notifyDashboardUpdate();
    res.json({ message: 'Visit scheduled successfully. QR will be sent via WhatsApp.', visitorId });
  } catch (error) {
